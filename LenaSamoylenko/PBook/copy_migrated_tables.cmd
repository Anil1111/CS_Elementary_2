...
ECHO `phonebook`	`name`	`PB`	`name`	`idName`	`idName`	`idName`, `FirstName`, `SecondName` >> %TMP%\wb_tables_to_migrate.txt
ECHO `phonebook`	`phone`	`PB`	`phone`	`id_phone`	`id_phone`	`idName`, `Phone`, `id_phone` >> %TMP%\wb_tables_to_migrate.txt
ECHO `phonebook`	`email`	`PB`	`email`	`id_mail`	`id_mail`	`idName`, `eMail`, `id_mail` >> %TMP%\wb_tables_to_migrate.txt
ECHO `phonebook`	`skype`	`PB`	`skype`	`id_skype`	`id_skype`	`idName`, `SkypeName`, `Password`, `id_skype` >> %TMP%\wb_tables_to_migrate.txt
ECHO `phonebook`	`address`	`PB`	`address`	`id_adress`	`id_adress`	`idName`, `City`, `Street`, `homeNumber`, `Flat`, `id_adress` >> %TMP%\wb_tables_to_migrate.txt
