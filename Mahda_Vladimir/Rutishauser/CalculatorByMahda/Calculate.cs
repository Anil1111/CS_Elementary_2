...
namespace CalculatorByMahda
{
    class Calculate
    {
        ...
        public static string MathFunction(int num1, int num2, string operation)
        {
            switch (operation)
            {
                ...
                case "/":
                    ...
                    if (num2 != 0) return (num1 / num2).ToString();
                    else
                    {
                        Console.WriteLine("Division by zero");
                        Console.ReadKey();
        ...
        public static string ParenthesesOpen(string inputExpression)
        {
            ...
            int resultStart = inputExpression.LastIndexOf('(', resultEnd);  //находим позицию открывающей скобки
